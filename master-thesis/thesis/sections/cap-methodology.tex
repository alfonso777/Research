...
de  coreografias, quaidade de serviço, SLA e monitoramento em coreografias de serviços Web. A dissertação de mestrado neste documento tem como
...
%. Para tanto, a seguir vamos definir o \textit{core process choreography BPMN} (baseado em \cite{Dijkman2007}).
  ...
  erros de comunicação, já que influem significantemente no tempo de resposta em uma coreografia. Nas mensagen, considera-se o  formato
...
%%TODO:precisa-se algum parágrafo para terminar a subseção?, dica: definição de requerimentos de QoS
...
\section{Simulador de Coreografias de Serviços}
  Atualmente,  implementar e executar uma coreografia de serviço real  é uma tarefa complexa já que as  tecnologias para suportar esse paradigma de
    composição de serviços  estão imaturas, especialmente pela falta de motores de execução cientes de coreografia~\cite{Kopp2010}. Assim,
  ...
  \subsection{Trabalhos Relacionados}
    ...
    Por conta disso, em~\cite{Guimaraes2012}, um dos autores desse artigo desenvolveu um simulador inicial para demonstrar que coreografias são mais eficientes
...
%Desde o SimGrid permite a simulação de ambientes distribuídos,  ele foi usado como base para implementar nosso simulador.
  ...
  cada bloco representa um componente do simulador. Um componente depende do componente ou vários componentes que estão embaixo dele.
...
dinâmicos no tempo de  execução ou de processamento de serviços.  Outros atributos como a largura de banda, latência, entre outros foram excluídos  para serem
 definidos como distribuições de probabilidade, porque os modelos que os suportam estão bem amarrados com a implementação do SimGrid.
 Contudo, para definir um comportamento   variável da largura de banda suportado pelo SimGrid usou-se a periodicidade. Assim, 
 diversos valores de largura são definidos em  intervalos de tempo que estão definidos em um  período de tiempo. Dessa maneira, no próximo Capítulo
...
\section{Especificação de SLAs Probabilísticos}
 ...
 $$F(x) = P_r( \delta \leq x) $$  
   Onde $P_r$ é a probabilidade correspondente a uma restrição sobre algúm parámetro de QoS. $\delta$ é um parâmetro de QoS (por exemplo, a 
  largura de banda efetiva), e $x \geq 0$. Então, o contrato ou restrição  será estabelecido
    ...
    Esta pesquisa  realizará a agregação das distribuição de probabiidade dos parâmetros de QoS de acordo com os padrões de interação de coreografia, de modo
    que a restrição ou contrato agregado  fique estabelecida no SLA de uma dupla de participantes da coreografia. Para estimar os quantis
...
%%TODO: descrição da técnica, da distancia e uso do one-side two sample de kolmogorov, etc
    ...
    É a função distribuição de probabilidade empírica, que define a proporção das amostras dos valores medidos coletado pelo monitor do
    parâmetro $p$ que são menores que $x$ dentro do conjunto $\Delta$. Daí, informalmente a restrição é cumprida se:
...
Seja $F_S$ a função de distribuição cumulativa prometida pelo contrato de um serviço $S$, e seja $F'_S$ a função de distribuição cumulativa atual
 do serviço $S$. O problema é  decidir entre duas hipóteses:
  ...
  %%\begin{equation*}
    %%\begin{multline}
	  H_0  : \forall x, F_S(x) >=F'_S(x)   \notag\\
	   contra: \notag\\
	  H_1 :  \exists x, F_S(x) < F'_S(x) 
   %\end{multline}
  %%\end{equation*}
  \end{align}
...
O teste de Kolmogorov-Smirnov de um lado de dois amostras~\footnote{Tradução de ``One-sided two-sample Kolmogorov-Smirnov test'' } é 
utilizado para aceitar ou rejeitar $H:0$.
      
      \begin{equation}
	[d,p] = kstest(X_{contract}, X_{monitoring}, KS_{side})
      \end{equation}

Onde $kstest$ é a função que desempenha o teste de Kolmogorov-Smirnov de um lado de dois amostras, que compara as distribuições
dos valores nos vetores $X_{contract}$ e $X_{monitoring}$. $X_{contract}$ corresponde ao contrato e $X_{monitoring}$ é o conjunto de amostras dos valores
...
A hipótese nula consiste em $X_{contract}$ e $X_{monitoring}$ são da mesma distribuição continua. A hipótese alternativa consite em que
$X_{contract}$ e $X_{monitoring}$ são de  diferentes distribuições. %%The result h is 1 if the test rejects the null hypothesis at the 5% significance level; 0 otherwise.
 $KS_{side}$ é um parametro cujo valor pode ser \emph{greater}, \emph{less} ou \emph{two-sided}. Para a detecção de violações usamos os valores $greater$ e $less$.
 $d$ é a distancia entre as duas distribuições cumulativas de $X_{contract}$ e $X_{monitoring}$ e $p$ é o ``\textit{p-value}'' do teste estatístico que tem valores
entre $[0,1]$.
...
Então, usa-se:
 ...
 para aceitar ou rejeitar $H_0$ (detectar uma violação) com o  valor \textit{greater} para o $KS_{side}$. Daí, $H_0$ é rajeitado quando o $p^{+}<\alpha$ é pequeno, onde
 geralmente é $\alpha = 0.05$, e assim dectecta-se uma violação do contrato.
...
Desse modo,  para cumprir o contrato a regra a seguir tem que ser verdadeira:
%  \begin{equation}
...
Onde $\lambda$ define a zona de tolerância da flutuação entre distribuições de probabilidade.
...
As principais caracterisiticas e  motivos para usar  o test de Kolmogorov-Smirnov (KS) são~\cite{Simard2011}:
  \begin{itemize}
    \item É um teste não-paramétrico, que compara as distribuições cumulativas de dois conjuntos de dados.
    \item	Dado que o teste é não-paramétrico, não assume que os dados são recolhidos a partir de distribuições gaussianas (ou quaisquer outras distribuições definidas).
    \item	Os resultados não mudam se transformam todos os valores para logaritmos ou recíprocos ou qualquer transformação. O teste de KS relata  a diferença máxima
...
%%TODO: algoritmo de deteção de violações
...
de  um subconjunto do total de elementos da especificação de coreografia em BPMN 2 para uma  GSPN  (Rede de Petri  Estocásticas Generalizada).
...
chamado de \textit{ChorSim}. Construiu-se o simulador de  coreografias para alavancar a pesquisa de QoS em coreografias de serviços, já que
