...
O objetivo deste capítulo é mostrar exemplos e cenários onde a utilização de  coreografias fazem sentido. Além disso, apresenta-se também
...
\section{Cénarios}
 ...
 Montage foi desenvolvido para apoiar a investigação científica. Ele permite aos astrônomos criar imagens de regiões do céu que são 
...
Montagem ilustra várias características de workflows científicos em uso intensivo de dados. Montagem pode resultar em enormes
 necessidades de fluxo de dados. Os dados intermdiários podem ser de três vezes o tamanho dos dados de entrada, por exemplo, um mosaico céu 
...
O  projeto Pegasus \footnote{Projeto Pegasus: \url{http://pegasus.isi.edu} } é um instrumento de gestão de fluxos de trabalho para construir
 e habilitar os fluxos de trabalho usados no Montage~\cite{aa}.

 \subsection*{Calculando o \textit{redshift}}

Este cenário foi tomado dos casos de uso científicos do AstroGrid~\cite{aaa}  e envolve recuperação e análise de dados em grande escala a partir
...
larguras de banda, extraindo objetos de interesse e calcular o \textit{redshift} relativa de cada objeto.
 ...
 \begin{figure}[!h]
      ...
      \caption{ Cenário de \textit{redshift} do AstroGrid: (a) orquestração, (b) coreografia. }
...
para extrair todas as imagens dentro das coordenadas indicadas que são retornadas ao cientista. As imagens são concatenadas e enviadas 
para o serviço ``SEextractor'',que é uma ferramenta para processamento. Sextractor digitaliza cada imagem, por sua vez, e usa um 
...
utilizada como entrada para  o algoritmo denominado ``HyperZ2'' que calcula os \textit{redshifts} fotométricos e os anexa a cada valor da 
tabela usada como entrada. Esta  tabela final consiste em arquivos multibanda contendo a posição solicitada, bem como uma tabela contendo
 para cada fonte todos os parâmetros  da saída do SExtrator e HyperZ, incluindo posições, magnitudes, classificação estelar e \textit{redshifts} 
fotométricos e intervalos de confiança, a tabela final é retornada par o usuário.


 \subsection*{Pegasus}

Pegasus (Planning for Execution in Grids) é um arcabouço que permite o mapeamento de fluxos de trabalho complexos em recursos distribuídos 
...
\begin{itemize}
 \item \textbf{Astronomia}: Montage, Galactic Plane.
 \item \textbf{Bioinformática}: DNA sequencing, Epigenomics, SeqWare, Association Mapping and Population Genetics in Vervets.  
 \item \textbf{Análise do genoma}: GADU (the Genome Analysis and Database Update system).
 \item \textbf{Modelagem do clima}: Climate Ensamble
 \item \textbf{Ciência para terremotos}: Southern California Earthquake Center - CyberShake and Broadband.
 \item \textbf{Heliosismologia}: Solar Dynamics Observatory (SDO).
 %\item \textit{Chemistry}: Biochemistry.
 %\item  Neuroscience: Telescience Project.
 \item \textit{Física}: Laser Interferometer Gravitational Wave Observatory (LIGO), Molecular Thermodynamics.
 \item \textbf{Ciencia do Oceano}: Predições no oceano.
...
Assim, as coreografia de serviços podem ser aplicadas nos cenários de fluxo de trabalho uma vez que esta acima cenários são fluxos de 
trabalho de dados intensivos e complexos ea coreografia abordagem pode ser melhorar vários aspectos deles.

 \subsection*{Uma rede de valor do negócio}

Arfa está visitando ULM (ou algum outro lugar turístico). Ela está gravando filmes e capturando fotos com uma câmera do seu telefone móvel. O 
...
online assim que ela completa uma gravação. Ela também está muito animada para compartilhar suas experiências com a sua família e amigos. Portanto, 
...
serviços, chamados de serviços de hospedagem de vídeo e serviço de pós-foto. O serviço de hospedagem de vídeo baixa o vídeo 
 do dispositivo móvel, e o melhorar e  arquivos. Qualquer usuário autenticado pode reproduzir o vídeo como no \textit{Youtube}. O serviço
...
capaz de fazer a sua tarefa mediante a contratação de  dois serviços, o \textit{Blog-Service} e o \textit{MMS-Service}. O  \textit{Blog-Service}
 ...
 o seu endereço exato. O \textit{MMS-Service} envia as imagens selecionadas para os amigos em seus telefones móveis.

  \begin{figure}[!h]
      \centering
      \includegraphics[width=0.8\textwidth]{figures/SLA-Choreography_Scenario.png}
      \caption{ Coreografia de SLA da rede de valor de negócio }
...
Um tipo particular de serviço está ganhando interesse em aplicações avançados é acerca do provisionamento de conteúdo em tempo real. O
...
Eles lidam tanto com a QoS de serviços Web e com o QoS dos objetos entregues, ou seja, o conteúdo de \textit{streaming}
 (imagens, vídeos, texto e som, e assim por diante).

O arcabouço em \cite{Buccafurri2008a}, lida com os processos interagindo com diferentes actores e oferecendo serviços de valor 
