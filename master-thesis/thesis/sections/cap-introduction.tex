...
Esta pesquisa  propõe  um monitoramento de coreografias de serviços Web seja baseado em restrições de atributos de QoS de maneira probabilística
...
desempenho e de rede. Dessa maneira, o monitor é responsável pela coleta e medição dos atributos de QoS dos serviços individuais, e agregá-los de acordo à
compsição e finalmente verifica a existência de violação de alguma restrição de QoS segundo como está definido no SLA.
...
\begin{enumerate}
  ...
  \item Definição de requisitos de QoS entre os serviços participantes que interagem em uma coreografia.
 ...
 %a definição dos SLAs entre toda dupla de participantes que interagem na coreografia e a configuração de monitoramento.
  \item Realização do monitoramento da coreografia, calculando  as distribuições de probabilidade dos parâmetros de QoS.
\end{enumerate}
...
etapa (2) se definem requisitos de QoS baseados em avaliações de desempenho de coreografias usando modelos de falhas probabilísticos de
 atributos de QoS. Com base nose requsitos de QoS  estebelecem-se SLAs probabilísticos. Para tanto utilizaram-se duas abordagens: 
...
parâmetros de QoS), e os compara com os quantis definidos nos SLAs para verificar alguma violação do contrato. Os parâmetros de QoS que
 serão utilizados são o tempo de resposta, o atraso de rede e a largura de banda efetiva das interações  entre os serviços.%latência de rede e a largura de banda. 
...
pesquisa.%, porque antes disso, é fundamental detectar as falhas por meio do monitoramento da coreografia. %para realizar uma adaptação ou  uma auto-cura (\textit{self-healing}). %Também não inclui predição de QoS baseado no histórico, mas isto será
 ...
 \begin{itemize}
    ...
    \item O projeto Baile~\footnote{\url{http://ccsl.ime.usp.br/baile/}} - Habilitando coreografias de serviços de núvem escaláveis.
