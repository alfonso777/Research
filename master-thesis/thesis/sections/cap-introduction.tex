...
para arquiteturas de software  e modelos de computação orientados a serviços, o que é chamado de SOC. SOC é um paradigma que
 utiliza serviços como base para construir e dar suporte a um desenvolvimento
...
Essas composições podem ser realizadas principalmente de duas formas: orquestração e coreografia, das quais
 a coreografia de serviços é mais colaborativa e especifica a composição desde uma perspectiva global. 
...
A orquestração de serviços é uma
...
Além disso, devido ao número crescente de dispositivos móveis que se conectam à Internet, uma abordagem orientada a serviços centralizada como a orquestração
...
é possível alavancar a seleção, composição, adaptação, auto-cura (\textit{self-healing}) e otimização de
 serviços Web baseadas em QoS~\cite{Rosenberg2009}.
...
Portanto, uma melhor forma de representar os requisitos de QoS é através de contratos não rígidos e baseados em probabilidade. Neste caso os requisitos estipulam
distribuições de probabilidade relacionadas com os valores dos
requisitos de QoS, por exemplo, atraso $<\ 10ms$ em 95\% das vezes em que a métrica for
...
Atualmente,  implementar e executar uma coreografia de serviços real  é uma tarefa complexa já que a  tecnologia para dar suporte a esse paradigma de
...
e é desenvolvido acima de um simulador para realizar  \textit{enactment} de coreografias de serviços. Tal simulador possui suporte de QoS, principalmente
atributos relacionados com tempo de processamento, tempo de resposta e atributos de rede. O monitor é responsável pela coleta e medição dos atributos de QoS dos
 serviços individuais, e por agregar as medidas de acordo com a
composição e, finalmente, por verificar a existência de violação de alguma restrição de QoS segundo contratos pré-estabelecidos.
...
\begin{itemize}
 \item Propor mecanismos de modo a detectar violações de SLAs %em coreografias de serviços Web baseadas em restrições probabilísticas  de QoS .
...
\begin{itemize}
 ...
 %baseados em restrições que utilizam distribuições de probabilidade. % em lugar de restrições rígidas (\emph{hard constraints}) que usam valores absolutos.
  ...
  \item Realizar avaliações do mecanismo de monitoramento de coreografias. % na implementação.
...
\begin{enumerate}
  \item Desenvolvimento de um simulador de coreografias de serviços Web com suporte à definição de atributos de QoS.
...
%medidos dos serviços Web e da composição  dos participantes
 Na etapa (1) construiu-se  um simulador de coreografias por conta da falta de implementações para rodar coreografias com suporte a QoS. Na
etapa (2) se definem requisitos de QoS baseados em avaliações de desempenho de coreografias usando modelos de falhas probabilísticos de
 atributos de QoS. Com base nesses requisitos de QoS se estabelecem os contratos  probabilísticos. Foram utilizadas duas abordagens
 para definir requisitos de QoS:
 %ou contratos não rígidos  utilizam os quantis das distribuições de probabilidade de um determinado parâmetro de QoS.
             \begin{itemize}
                  \item \textbf{Analítica}:  Usando GSPN como representação intermediária
...
Na etapa (3),  o monitoramento utiliza os quantis das distribuições de
probabilidade empíricas, de acordo com os valores medidos dos
...
pesquisa.%, porque antes disso, é fundamental detectar as falhas por meio do monitoramento da coreografia. %para realizar uma adaptação ou  uma auto-cura (\textit{self-healing}). %Também não inclui predição de QoS baseado no histórico, mas isto será
 ...
 \begin{itemize}
    ...
    \item O projeto Baile~\footnote{Projeto Baile: \url{http://ccsl.ime.usp.br/baile/}} - Habilitando coreografias escaláveis de serviços na nuvem.
    %Enabling Scalable Cloud Service Choreographies
 \end{itemize}

%TODO: escrever mais acerca a relacao da tese com os choreos, deliverables, etc
...
No Capítulo~\ref{cap:methodology} são descritas as técnicas e algoritmos para atingir os objetivos desta pesquisa.
 O Capítulo~\ref{cap:results} descreve os experimentos realizados e analisa os resultados encontrados.
