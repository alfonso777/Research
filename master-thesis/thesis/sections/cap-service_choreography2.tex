...
Como explicado no Capítulo 2, orquestração e coreografia são dois tipos de composição de serviços.
...
protocolo de interação entre os diversos serviços participantes é descrito de uma perspectiva global. Em~\cite{Barros2005},
apresenta-se  uma definição genérica, na qual uma coreografia de serviços é um processo colaborativo que envolve múltiplos
...
Diversas propostas para definir e especificar coreografias têm surgido. A seguir são apresentadas as definições de coreografia de serviços segundo
...
definição explícita de coreografia de serviços, mas a palavra
 coreografia e as palavras derivadas aparecem no documento. Coreografia de mensagens é descrita como a ordem na qual as mensagens podem ser
 enviados ou recebidos em uma dada troca de mensagens, as regras que governam tal ordem e quando as mensagens começam e terminam.
    \item \textbf{WS-CDL}: Em \cite{WSDL2005}, coreografia é definida como um contrato multi-participante visto de uma perspectiva Global. Na sua
 especificação, não há uma definição explícita de coreografia de serviços web, mas
 a camada de coreografia é descrita como o componente que descreve colaborações entre participantes, definindo
de um ponto de vista global seus comportamentos e onde a troca de informação
 acontece.

  \item \textbf{BPMN}: No documento de especificação do BPMN 2.0 \footnote{ BPMN 2.0: \url{http://www.bpmn.org/} } uma
 coreografia é definida como uma sequência ordenada de troca de
 mensagens B2B (Negócio a Negócio) entre dois ou mais participantes. Uma coreografia não possui controlador central,
 entidade responsável e nem observador do processo.

  \item \textbf{BPEL4Chor}\cite{Decker2007}: Uma coreografia descreve a troca de mensagens entre serviços da perspectiva de um observador que é
 capaz de ver todas as interações e o fluxo de dependências .

  \item \textbf{Let's Dance}\cite{Zaha2006a}: Em uma perspectiva global (coreografia), as interações são descritas da perspectiva de uma coleção
 de serviços abstraídos como papéis.
...
%\begin{comment}
\section{Orquestração e Coreografia de serviços}

Tanto orquestrações quanto coreografias de serviços fundamentam a área da engenharia de software orientada a serviços, e assim, representam
diferentes escolhas de projeto na hora de implementar sistemas orientados a serviços \cite{Rosenberg2009}.
...
coreografia de serviços como o WS-CDL. É frequente o caso em que as duas linguagens
 são necessárias. Por exemplo, um fluxo de trabalho pode ser descrito como uma coreografia, e este
 pode se realizar como um conjunto de  fluxos de trabalho executáveis. Nesse cenário os serviços podem ser orquestrados, usando por exemplo WS-BPEL, de modo que
o comportamento externo observável é definido e gerado a partir de uma coreografia, usando por exemplo WS-CDL. Assim, a coreografia
é uma guia para o sistema todo, e a orquestração é uma maneira para realizar e implementar o sistema sem a introdução de um intermediário de
 serviços global \cite{Ross-talbot2005}.  %overall broker-service.
...
que vários participantes estejam envolvidos para que descrevam sua parte na interação. A coreografia acompanha a sequência de mensagens de
 múltiplos participantes e fontes, em vez de apenas um processo de negócio específico.


Como o objetivo desta pesquisa é propor mecanismos relacionados com coreografia de serviços Web, a partir deste ponto este texto focará nesse tipo
 de composição.
...
Um processo de negócio inter-organizacional (IOBP) é um grupo organizado de atividades realizadas por duas ou mais organizações para atingir
um objetivo de negócio. Como consequência disso, a modelagem e o projeto de processos de negócios usados dentro de uma organização tem que
 ser melhorada e estendida para lidar com  particularidades de negócios inter-organizacionais~\cite{Bouchbout2011}.
...
processos de negócio inter-organizacionais. A ideia por trás das coreografias está relacionado ao fato de que as entidades de negócios
 frequentemente consideram seus processos intra-organizacionais como ativos, por exemplo, visão baseada em recursos em gestão estratégica~\cite{Mendling2005}.
...
Nesse contexto, as coreografias são uma abordagem para modelar processos ou \textit{workflows} inter-organizacionais do ponto de
 vista global (ver Figura~\ref{fig:Choreography_BP}). Nesse esquema os processos ficam descritos de maneira ``privada''
...
uma linguagem de modelagem de coreografias $L$ como uma coleção de modelos de coreografias:
 $$L=\{(C,I)\ | \textit{C é uma coreografia e  I é sua implementação  mediante  serviços} \}$$
...
as ações locais, que são realizadas nos serviços individuais independentemente de outros serviços. As restrições de sequência restringem
...
síncrono e assíncrono. No caso do modelo assíncrono, é comum utilizar uma fila \textit{FIFO}\footnote{FIFO: Acrônimo em inglês para Primeiro em Entrar Primeiro em  Sair.  } no receptor para armazenar por ordem de chegada as mensagens que ainda não foram consumidas \cite{Mancioppi2010}. Dadas estas definições,
 os modelos de coreografia de serviços têm que lidar com os seguintes problemas \cite{Bultan2008}, \cite{Mancioppi2010}:
  ...
  \begin{itemize}
      ...
      \textit{peers} gerados coincidam com a especificação da coreografia \cite{Bultan2009}. Em outras palavras, \textit{realizability} é a possibilidade de
      que  uma  especificação de coreografia possa ser realizada pelos serviços ou participantes \cite{Su2007a}.
      ...
      realizada por algumas orquestrações e sintetizar uma combinação de orquestrações se for possível \cite{Sun2010}.
      ...
      confiável. O conjunto de serviços compatíveis não somente depende da sequência de mensagens, mas também depende de propriedades quantitativas
 ...
 associados: autômatos de estado finito, redes de \textit{Petri} e álgebra de processos.
   ...
   \subsection{Modelos baseados em Autômatos}
    ...
    Os modelos baseado em autômatos representam a coreografia e a implementação de serviços como uma maquina de estado finito, isto é, especificam uma
     coreografia por meio de estados e transições. Desta maneira, capturam explicitamente um instantâneo da execução do serviço composto como um estado, e os
    comportamentos locais e globais podem ser capturados como uma sequência de estados, nos quais cada estado de transição está associado
    a uma mensagem ou uma atividade \cite{Su2007a}.

    Este grupo de linguagens de modelagem de coreografias inclui: protocolos de conversação, serviços \textit{Mealy}~\cite{Bultan2003}, diagramas de
    colaboração UML~\cite{Bultan2008a}, e o modelo de
    composição de serviços Colombo~\cite{Berardi2005}. A linguagem \textit{Let's Dance} \cite{Zaha2006a} também pertence a esse grupo, e
    fornece um conjunto de primitivas de restrição de sequências
    para permitir a especificação de uma coreografia com uma notação gráfica.
   ...
   \subsection{Modelos baseados em Redes de Petri}
    São amplamente utilizados para modelar, entre outras coisas, fluxos de controle, e por conta disso, são candidatos adequados para linguagens de modelagem
    de coreografias. Por exemplo, IPN (\textit{Interaction Petri Net}), uma linguagem de modelagem de coreografias baseada em Redes de Petri, trata uma
    troca de mensagem como o disparo de uma transição. O uso de redes de Petri permite que comunicações concorrentes sejam explicitamente separadas, ao
    ...
    redes de Petri  com entrada e saída. Os serviços ou participantes se comunicam com outros no modelo de mensagens síncronos,
   ...
   \subsection{Modelos baseados em Álgebra de Processos}
    São menos intuitivos e não possuem representação gráfica, e assim como os modelos baseados em redes de Petri, fornecem um formalismo para a
  verificação das propriedades de uma especificação. Existem várias propostas no
    desenvolvimento de linguagens de modelagem de coreografias baseadas em álgebra de processos \cite{Su2007a},
muitas dessas propostas estão baseada em cálculo Pi. O cálculo Pi é uma álgebra de processos que tem como intuito ser uma teoria formal para modelagem de
processos. Exemplos de linguagens deste tipo são XLANG, WS-CDL e as baseadas nela,
...
Os modelos de interação usam interações atômicas como blocos de construção  básicos e os fluxos de dados
...
O protocolo BPSS \footnote{BPSS: \textit{Business Process Specification Schema}} pode definir coreografias e
...
O WSFL é uma linguagem baseada em XML que consiste de várias vistas locais, também chamadas de
...
\textit{Chor} \cite{Zongyan2007} e  %( subconjunto do WS-CDL),
 WSCDL+ \cite{Kang2007}. Contudo, atualmente a especificação do WS-CDL está descontinuada~\cite{WSCDLv12005}.%, entre outros. O \textit{BPEL gold} \cite{Engler2009}
...
Existem várias ferramentas e arcabouços para apoiar diversos aspectos da coreografia de
serviços, tais como modelagem, verificação e validação, simulação, síntese e mapeamento para processos executáveis
...
\textit{WS-CDL Eclipse} \cite{Douglas2005} e Pi4SOA \cite{Zhou2006} foram algumas das primeiras implementações
...
entre outros.  Essa plataforma utiliza o LCC (\textit{Lightweight Coordination Calculus}) \cite{Robertson2005} como a linguagem executável baseada
no \textit{Process-Calculus} para especificar e executar as coreografias. A sua arquitetura é capaz de lidar com a heterogeneidade
...
especialmente no nível da análise de domínios  e design de sistemas de alto nível. A notação herda e combina elementos a partir 
...
tais como  \textit{AND-split}, \textit{AND-join}, \textit{XOR-split} e \textit{XOR-join}. A Figura~\ref{fig:BPMN_CoreElements} mostra um subconjunto do núcleo de elementos de BPMN. Há objetos e
 fluxos de sequência. Um objeto pode ser um evento, uma atividade ou um \textit{gateway}. Uma fluxo de sequência liga dois objetos em uma BPD
...
Um evento pode sinalizar o início de um processo (evento de inicio), o final de um processo (evento de finalização), e pode
 também ocorrer durante o processo (evento intermediário). Um evento de mensagem é usado para enviar ou
 receber uma mensagem. Um evento temporizador (\textit{timer}) indica um tempo ou data específico
 a ser atingido, e um evento de erro sinaliza um erro a ser detectado durante a execução de um processo.
...
Uma atividade pode ser uma tarefa ou um subprocesso. Uma tarefa é uma atividade atômica e significa trabalho a ser executado dentro
 ...
 exemplo, uma tarefa de recebimento é utilizada quando um processo aguarda por uma mensagem de um participante externo. Um
...
seguir a descrição dos principais tipos de \textit{gateways}:
  ...
  \begin{itemize}
    \item \textbf{De divisão em  paralelo (\textit{AND-split})}: para a criação de fluxos de sequência concorrentes.
    \item \textbf{De junção paralela (\textit{AND-join})}: para a sincronização simultânea de vários fluxos de sequência.
    \item \textbf{De escolha XOR baseados  em dados/eventos (\textit{data/event-based XOR decision})}: para a seleção de um conjunto
      ...
      \textit{XOR-split}) ou de eventos externos (\textit{event-based}).
    \item \textbf{De junção XOR (\textit{XOR-join})}: para unir um conjunto de fluxos de sequência mutuamente excludentes dentro de um
      único fluxo de sequência.
  \end{itemize}

%Em particular, um \textit{gateway} de escolha XOR baseado em eventos deve ser seguido por alguma tarefa de recebimento  ou  evento intermediário
...
Descrições de comportamento dos participantes são primeiro utilizadas como vistas de coreografia de acordo com a
 perspectiva de um participante individual. Depois disso, elas servem como especificações para a implementação de novos serviços ou
 a adaptação de serviços existentes~\cite{Pfitzner2008}.

Como já descrito na Seção~\ref{sec:chor_languages}, existem duas maneiras de especificar coreografias de serviços, com modelos de
 interação e com modelos de interconexão. 
Em ambos os estilos de modelagem, os participantes interagem entre si e as atividades são conectadas. O modelo 
de interconexão conecta atividades de comunicação pertencentes a dois participantes. Assim, cada troca de mensagem é
 expressa usando  uma conexão entre os participantes \cite{Kopp2011}. 
O modelo de interação expressa cada troca de mensagem como interações atômicas. Em geral, os 
...
%Essas duas formas de especificar coreografias podem ser consideradas como diferentes visões de uma mesma coreografia~\cite{Kopp2011}. O
...
Os modelos de interconexão modelam cada participante como um processo separado, e podem fornecer o comportamento interno dos participantes. Exemplos
...
comportamento do processo interno de um participante é modelado com elementos comuns do BPMN, por exemplo, atividades, eventos,
 e os fluxos de sequência~\cite{Kopp2011}. Alternativamente, os \textit{pools} podem ser descritos como caixas pretas. %Na discussão abaixo nos concentramos em piscinas que contêm processos internos
...
proposta de investimento. Cada participante é representado por meio de \textit{pools} nos quais se interconectam atividades.
...
%Os padrões de interação de serviços~\cite{Barros2005a} descrevem um conjunto de cenários recorrentes de coreografias. Eles variam
...
É importante observar que essa abordagem de modelagem leva as dependências de fluxos de controle a serem redundantes e ter  mais chances de 
 ...
 de entregar os bens adquiridos. O comprador, por outro lado, espera que as mercadorias possam ser entregues antes de ter pago. Ambos os parceiros iriam esperar indefinidamente - uma clássica situação de \textit{deadlock}. Os modelos 
de interação evitam esses  problemas por meio da  descrição de dependências de fluxos de controle entre as interações~\cite{Decker2008}.
...
por meio de troca de mensagens. O modelo de interação é considerado nesta dissertação.% \cite{Decker2008}.
...
O bloco básico de construção nos modelos de interação são as ``atividades'' de interação. Cada atividade descreve
 uma troca de mensagem entre dois participantes de maneira atômica na especificação de uma coreografia. Os
 modelos de interação fornecem uma visão de alto nível das colaborações e a maioria das linguagens de coreografia neste
...
uma ``tarefa de coreografia'' (\textit{choreography task}), das quais a última será levada em consideração nesta dissertação.

Uma tarefa de coreografia é uma atividade atômica em uma coregrafia de processos. Ela representa uma interação em que uma ou duas trocas de
 mensagens acontecem entre dois participantes. A Figura~\ref{fig:ChoreographyTask} mostra uma tarefa de coreografia, em que as duas faixas, ``Participant A'' e ``Participant B'', representam os participantes. A faixa em cor branca é do participante que inicia a interação e a faixa em cor escura é do participante dono
 da tarefa. A tarefa está representada pela faixa interna chamada de ``Choreography Task Name'' na figura.
...
As tarefas de coreografias são de um ou de dois sentidos. A tarefa de coreografia da Figura~\ref{fig:ChoreographyTask} é de um sentido e
captura interações onde ocorrem apenas  um envio de mensagem sem resposta, o que pode ser visualizado melhor no seu respectivo diagrama
 de colaboração (Figura~\ref{fig:ChoreographyTask1}). Uma tarefa de coreografia de dois sentidos captura interações atômicas em que existe resposta.
  A sua representação contêm as duas mensagens enviadas, como mostrado no diagrama de colaboração da Figura~\ref{fig:ChoreographyTask2}. A mensagem de cor branca é a iniciadora e a mensagem de cor escura é a resposta.
 ...
 %denominadas como atividades de coreografia. 
  ...
  \begin{figure}[!h]
      ...
      %\includegraphics[width=0.7\textwidth]{./figures/BPMNBasicChoroegraphy.png}
      %\caption{Elementos BPMN para modelar coreografias de processos}
      \caption{Uma tarefa de coreografia de um sentido e seu diagrama de colaboração \cite{OMG2010}}
  ...
  \begin{figure}[!h]
      ...
      %\includegraphics[width=0.7\textwidth]{./figures/BPMNBasicChoroegraphy.png}
      %\caption{Elementos BPMN para modelar coreografias de processos}
      \caption{Uma tarefa de coreografia de dois sentidos e seu diagrama de colaboração \cite{OMG2010}}
...
%Koppa2010
A Figura~\ref{fig:Example-InteractionChor} apresenta o modelo de interação equivalente ao modelo de interconexão apresentado na 
 Figura~\ref{fig:Example-InterconectionChor}. Nesse caso, o fluxo de controle é representado entre os \textit{pools} e as 
atividades de mensagens locais foram substituídas por atividades de interações atômicas. 
...
%---------------------------------------------%
  ...
  Neste capítulo foram apresentados aspectos chave em coreografia de serviços Web. Para efetuar o \textit{enactment} de coreografias,
 precisam serem mapeadas para processos  de negócio executáveis tais como BPEL. Assim,  o \emph{enactment} de coreografias depende
  do motor de processos executáveis, mas, atualmente, esses motores não são cientes de coreografia. Em consequência, deveriam existir motores
  cientes dos aspectos envolvidos nas coreografias de serviços Web. Assim, as atuais implementações de ESB não
  oferecem características específicas das coreografias (\cite{Kopp2008}),  por exemplo,
  as implementações de ESB não verificam se as mensagens estão trafegando de acordo à descrição da coreografia, isto é, não há condições de realizar um monitoramento de forma trivial.
  ...
  Portanto, já que os atuais motores não são cientes de de coreografias, neste trabalho desenvolveu-se um simulador para possibilitar o
 \textit{enactment} de coreografias. O simulador possibilita a pesquisa em QoS e monitoramento em coreografias, os quais  são alvo desta dissertação.
