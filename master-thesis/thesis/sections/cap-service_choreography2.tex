...
No entanto, as empresas querem se beneficiar de fortes colaboração com os seus parceiros (participantes).
...
Alé disso, a modelagem de processos de negócio inter-organizacionais envolve novos desafios, principalmente, a capacidade de lidar com 
...
%participantes de negócios são mutuamente independentes (autônomos e heterogêneos) \cite{Telang2011}
 ...
 \begin{figure}[!h]
      \centering
      \includegraphics[width=1.0\textwidth]{./figures/Choreography1.png}
...
\subsection{BPMN}
O BPMN (Notação de Modelagem de Processos de Negócio) surgiu como um padrão para notação para capturar  processos de negócio, 
...
O BPMN é uma linguagem independente de implementação e, especialmente, permite a definição de coreografias interligando diferentes
...
termos ``modelo de interconexão'' e ``modelo de interacção''  são derivadas da maneira como cada paradigma de modelagem
 expressa a troca de mensagens  e não do fato de se as atividades são conectadas ou são interações gerais
 entre os processos são apresentados \cite{Kopp2011}. 
...
  incluem a abordagem Open Net~\cite{aa}. 
...
%Koppa2010
A Figura ~\ref{fig:Exemplo-InterconectionChor} apresenta um exemplo de  coreografia especificada de acordo com o modelo
de interconexão. O exemplo é acerca de oferta investimentos e está especfificada utilizando a notação BPMN 1.2. Primeiro, 
...
modelo de interconexão, ele apenas fornece direto suporte para um conjunto limitado de padrões de interação de serviços~\cite{Decker2007a}.
...
  entregar os bens adquiridos. O comprador, por outro lado, espera que as mercadorias possam ser entregues antes de realmente pagado
  eles. Ambos os parceiros iria esperar indefinidamente - uma clásica situação de impasse (\textit{deadlock}). Os modelos 
...
Alguns elementos BPMN são comuns para ambos os diagramas de processos normais  e diagramas de coreografias (modelo de interação), bem como 
 colaborações (modelos de interconexão). Tais elementos comuns incluem principalemente eventos, gateways, e fluxos de sequência e não incluem
os ``pools''.  
...
Uma atividade de coreografia pode se referis a elementos como  ``subcoreografia'',  ``chamada a outra coreografia'' (\textit{callchoreography}) e 
uma ``tarefa de coreografia'' (\textit{choreography task}), das quais esta última será levada em consideração nesta pesquisa.
...
Uma tarefa de coreografia é uma atividade atômica em uma coregrafia de processos. Representa uma interação em que uma ou duas trocas de
 ...
 representam os participantes, a banda em cor branca é do participante iniciador da interação e banda escura é do participante dono
...
\begin{figure}[!h]
      ...
      %\caption{Elementos BPMN para modelar coreografias de processos}
      \caption{Representação de uma tarefa de coreografia }
...
captura interações onde ocorrem apenas  um envio de mensagem sem resposta, o que pode ser visualizado melhor no seu respectivo diagrama
 ...
 %denominadas como atividades de coreografia. 
  ...
  \begin{figure}[!h]
      ...
      %\includegraphics[width=0.7\textwidth]{./figures/BPMNBasicChoroegraphy.png}
      %\caption{Elementos BPMN para modelar coreografias de processos}
      \caption{Uma tarefa de coreografia simples e seu equivalente em diagrama de colaboração }
...
\begin{comment}
  \begin{figure}[!h]
      ...
      %\includegraphics[width=0.7\textwidth]{./figures/BPMNBasicChoroegraphy.png}
      %\caption{Elementos BPMN para modelar coreografias de processos}
      \caption{Atividades de Coreografia em BPMN: interações atômicas }
...
%---------------------------------------------%
  ...
  do motor de processos executáveis, mas que não têm consciência de coreografia. Em consequência, deveriam existir motores
  com consciência dos aspectos envolvidos nas coreografias de serviços Web. Assim, as atuais implementações de ESB (abordagem utilizada pelos motores de BPEL) não
  ...
  ESB com consciência de coreografias. WS-CDL+ \cite{Kang2007} é outro trabalho que visa ser um motor de execução de coreografias.
 ...
 ``enactment'' de coreografias. Assim, também o simulador possibilitará a pesquisa em QoS e monitoramento em coreografias, os quais  são alvo deste trabalho.
