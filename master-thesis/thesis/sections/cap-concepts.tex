...
Este capítulo apresenta conceitos básicos necessários para a
...
B2E.  A organização responsável pela definição de normativas e
padrões que permitem que sistemas desenvolvidos em diferentes plataformas e diferentes linguagens de programação possam interagir %, e garantir esse comportamento,
é a WS-I~\cite{wsi2005}(\textit{Web Services Interoperability
Organization}).
...
A arquitetura de serviços Web permite que os serviços
...
Os serviços Web são aplicações auto-contidas e modulares que podem ser: 
...
A Figura~\ref{fig:WS_Triangle} ilustra os componentes que estão envolvidos no modelo de uma  arquitetura de serviços Web. A 
...
\begin{itemize}
  ...
  pelo provedor de serviços. Obviamente, a execução é realizada em uma plataforma acessível via rede. %O serviço é descrito
...
\begin{itemize}
  \item \textbf{Publicar/Cancelar}: Um provedor de serviços pode publicar um determinado serviço comercial em um ou mais registros de serviços
 e cancelar essa publicação em qualquer momento.
  \item \textbf{Procurar}: Os  solicitantes dos serviços podem interagir com um ou mais registros para encontrar um conjunto de serviços
...
Atualmente, as aplicações que estão sendo desenvolvidas têm a capacidade de procurar  e selecionar serviços de forma dinâmica em tempo real, com base
 ...
 o sistema automaticamente escolhe o serviço que melhor se adapte às
necessidades do solicitante~\cite{Wang2004}.% e, portanto, o desempenho do sistema será acrescentado \cite{Wang2004}.
...
\begin{itemize}
  \item \textbf{Interoperabilidade:} Os serviços Web podem interagir com outro serviços. O protocolo padrão SOAP permite que os serviços
possam ser oferecidos ou utilizados independentemente da linguagem ou ambiente em que são desenvolvidos.
...
o cliente e acessar os serviços Web. Por exemplo, uma máquina de venda de refrigerantes pode se comunicar com o serviço Web de um provedor local
 e solicitar uma ordem de fornecimento através de uma rede de acesso
sem fio existente.
  \item \textbf{Barreira mínima de participação:} Os conceitos por trás dos serviços Web são não dífíceis de compreender e existe uma gama de
ferramentas de desenvolvimento, como os oferecidos pela IBM, \textit{Sun Microsystems}, \textit{Apache}, \textit{Systinet}, entre
outras. Tais ferramentas permitem que os desenvolvedores criem e implementem serviços Web de maneira rápida.
  ...
  REST, conhecidos como serviços Web \textit{RESTFul}~\cite{Peng2009}, e tecnologias derivadas dos serviços Web.
\end{itemize}
...
A infraestrutura mínima exigida pelos serviços  Web pode ser definida em termos de~\cite{Pelechano2005}: 
\begin{itemize}
  \item O que é transferido pela rede: Formatos e protocolos de comunicação. 
  \item O que é descrito para ser transferido via rede: Linguagens de descrição de serviços. 
  \item O que permite encontrar e armazenar essas descrições: Descoberta de serviços. 
\end{itemize}

Os serviços Web baseiam-se nas seguintes normativas e protocolos~\cite{WSAlonso2004}: 

\begin{itemize}
  \item \textbf{XML}~\cite{W3CXML1998}, publicado em 1998, mudou e impactou a maneira de estruturar, descrever e
 trocar informação. Independentemente
 das muitas maneiras atuais de usar XML, ainda muitas das tecnologias de serviços Web são baseadas em XML apesar do acrescentamento dos serviços RESTful.
 É o padrão central desta arquitetura, 
sobre a qual o resto de padrões  se apoiam. O projeto  do XML deriva de duas fontes principais: SGML (\emph{Standard Generalized Markup
 Language})~\cite{ISOSGML1986}. %e HTML(\emph{HyperText Markup Language})~\cite{ISOHTML2000}.
 
  \item \textbf{UDDI}~\cite{UDDI2005}, é um diretório que contém um registro/repositório de
...
os clientes; e por outro lado, facilitando a ligação dinâmica de serviços Web, permitindo consultar referências e acessar serviços de interesse. 
  ...
  \item \textbf{SOAP}~\cite{W3CSOAP2003}, é um padrão do W3C\footnote{\url{www.w3.org/} }, que define
  um protocolo que dá suporte para a interação (dados + funcionalidade) entre aplicações em ambientes heterogêneos distribuídos. SOAP define a 
 forma de organizar informações em XML para permitir a troca entre diferentes sistemas. O protocolo SOAP simplifica
 o acesso a objetos, permitindo que as aplicações possam invocar métodos de objetos ou funções que residem  em sistemas remotos.
  ...
  \item \textbf{WSDL}~\cite{W3CWSDL2001}, criada pela IBM, Microsoft e Ariba, tem papel e propósito semelhante à IDL 
(\emph{Interface Definition Language}) das plataformas de \textit{middleware}. Um arquivo WSDL é um documento XML que descreve os serviços Web, particularmente suas
 interfaces. Uma  característica que o diferencia da IDL, é que a WSDL deve definir os mecanismos de acesso (protocolos) para os serviços web. Outra
característica distintiva é a  necessidade de definir na especificação a localização  de um serviço (\emph{endpoints}). A separação das interfaces e dos
 protocolos, e a necessidade de incluir as informações de localização, permite a definição de especificações modulares. % WSDL permite definir interfaces de
...
%Na Figura \ref{fig:WS_model} é apresentado o diagrama de arquitetura dos sistemas baseados em serviços Web, onde se representam os padrões e tecnologias descritas. 
%    \begin{figure}[!h]
%      \centering
%      \includegraphics[width=.70\textwidth]{figures/webServiceModel.png}
...
O grau de consenso da indústria no escopo dos protocolos dos serviços Web é elevado (pelo menos em termos de infraestrutura básica). Apesar de
...
paradigmas de programação, metodologias de desenvolvimento e também os modelos de computação tanto no hardware quanto via rede, convergindo para 
a SOC.
...
evolutiva, de baixo custo  e de fácil composição de aplicações distribuídas. Os serviços desempenham funções que podem variar de responder requisições simples até a execução
 de processos de negócios sofisticados que exijam relações P2P entre várias camadas de consumidores e provedores de serviços.

Trechos de código e  componentes de algum aplicativo implantado em um sistema pode ser reutilizado e transformado
 em um serviço disponibilizado na rede. Os  serviços geralmente são implementados de maneira
 independente do contexto em que serão utilizados. Isso significa que entre o provedor e o consumidor de serviços há um baixo 
acoplamento.
...
A abordagem  orientada a serviços  é independente de linguagens de programação ou sistemas operacionais. Isso permite que
 as organizações exponham suas funcionalidades essenciais por meio da Internet e de uma variedade de tecnologias de redes usando, por exemplo,  
linguagens baseadas em XML e protocolos padrão, e
implementando uma interface auto-descritiva. A promessa visionária de SOC consiste em  construir facilmente  componentes de aplicação dentro de uma  rede de serviços
...
A chave para concretizar a visão da SOC é a SOA. A SOA descreve um maneira lógica com o objetivo de projetar sistemas de software para
fornecer serviços para aplicações de usuário final ou para outros serviços distribuídos em uma rede, por meio de interfaces publicadas e detectáveis, isto é, que possam
...
A SOA orienta os requisitos de baixo acoplamento, baseando-se em padrões e em computação
distribuída independente de protocolos~\cite{Papazoglou2007}. A Figura~\ref{fig:soc_roadmap} mostra um roteiro para o desenvolvimento de SOA segundo~\cite{PapazoglouSOC2007a}). A pirâmide da figura
separa as funcionalidades da arquitetura em três planos. Serviços básicos na base da pirâmide, composição de serviços no meio e gerenciamento de serviços
 com monitoramento no topo da pirâmide. Esta divisão lógica está baseada na necessidade de separar:

\begin{itemize}
 \item Capacidades de serviços básicos fornecidos por uma infraestrutura de \emph{middleware}, e uma SOA convencional de funcionalidades de serviços avançadas necessárias para dinamicamente compor serviços.
...
\end{itemize}
   ...
   \begin{figure}[!h]
      ...
      \caption{Roteiro para o desenvolvimento de SOA \cite{PapazoglouSOC2007a} }
...
Para a construção de aplicações orientadas a serviços usando SOA, também foi desenvolvido uma série de padrões de projeto de software \cite{Gamma1994}. Com o uso
 de padrões, que podem ser usados com qualquer metodologia, a SOA resultante irá conter um maior nível de abstração, garantindo consistência e melhor desempenho. 
Os padrões de projeto para construir uma arquitetura orientada a serviços podem ser divididos em cinco categorias \cite{Monday2003}: 

\begin{enumerate}
  \item
  \textbf{Aprendizagem}: Utilizado para compreender o ambiente dos serviços Web. Dentro desta categoria encontramos os padrões:
  \begin{itemize}
    \item Arquitetura Orientada a Serviços: O padrão que forma a arquitetura dos serviços Web.
    \item \textit{Architecture Adapter}: Um modelo genérico que facilita a comunicação entre arquiteturas.
    \item \textit{Service Directory}: facilita a transparência na localização de serviços, permitindo a criação de interfaces robustas para encontrar o serviço adequado.
\end{itemize}

  \item \textbf{Adaptação}: Utilizados para possibilitar o conhecimento do ambiente dos serviços web. Nesta categoria encontramos os padrões:
    \begin{itemize}
      \item \textit{Bussines Object}: Um \textit{business object} engloba um conceito de negócio no mundo real, como um cliente, uma empresa ou produto. Este padrão possibilita a transferência do conceito de objeto de negócios para o paradigma de Web Services.
      ...
      \begin{itemize}
	\item BPEL, proposto pela BEA Systems, IBM e Microsoft.
	\item BPMN, proposto por WebMethods, SeeBeyond, entre outras.	                                   
	\end{itemize}

      \item \textit{Bussines Object Collection:} Este padrão habilita a realização de composições de processos de negócios. 

      \item \textit{Asynchronous Business Process}: Este padrão é a evolução do padrão Business Process. 
  \end{itemize}

  \item \textbf{Alterações}: Utilizados para definições relacionadas com serviços baseados em eventos. Esses padrões são baseados em padrões tradicionais, como o Observer ou o padrão Publish / Subscribe. Nesta categoria encontramos:
    \begin{itemize}
	...
	mais comum de use deste padrão é em aplicações de EAI (Enterprise Application Integration) \cite{Gu2010}.
      \item \textit{Observer Services}: Este padrão representa a forma mais natural para detectar mudanças no ambiente e tomar ações.
	...
	nas notificações, o que permite que diferentes serviços possam enviar a mesma notificação. 
    \end{itemize}
  ...
  do serviços Web e a personalizar o ambiente para atender às necessidades da organização. Nesta categoria encontramos:
    \begin{itemize}
      ...
      o fluxo de negociações que pode ser obtido com o padrão \textit{publish/subscribe}.
      ...
      \item \textit{Faux Implementation}: É uma alternativa para resolver os problemas que surgem na utilização de eventos em serviços Web. É simplesmente um socket
      aberto que recebe as conexões e fornece as respostas para os diferentes eventos.
\end{itemize}

  \item \textbf{Flexibilidade}: Utilizados para criar serviços mais flexíveis e otimizados. Nesta categoria estão:
    \begin{itemize}
      \item \textit{Service Factory}: Permite a seleção de serviços e fornece flexibilidade na instanciação dos componentes que criam
	...
	utilizam os serviços Web, oferecendo um maior retorno de investimento às aplicações. 
      \item \textit{Data  Transfer  Object}: Este padrão fornece desempenho, permitindo coletar dados e enviar múltiplas informações em uma única chamada, reduzindo o número de conexões
	que o cliente tem que criar para comunicação com o servidor.
      \item \textit{Data  Transfer  Collection}:  Esta é uma extensão da primeira, dado que o padrão \textit{Data  Transfer  Object} pode ser aplicado a uma coleção de objetos
      ...
      do servidor aquilo que é necessário. Este padrão além de desempenho também oferece uma melhor utilização da largura de banda disponível na rede.
\end{itemize}

\end{enumerate}

Alguns padrões usam outros padrões, por exemplo, o padrão \textit{Business Process} utiliza o \textit{Business Object} e o \textit{Business Object Collection}. De forma similar, o \textit{Service-Oriented Architecture} faz uso do \textit{Service Directory} e do \textit{Architecture Adapter}. 
...
serviços para que ela possa ser facilmente compreendida pelos interessados. Por outro lado, as linguagens internas, ou linguagens de composição de serviços, são
...
colaboração  entre
 ...
 de processos. Esse consórcio publicou uma especificação para uma linguagem de descrição de processos chamada de BPML (\emph{Business Process Modeling Language}), que
...
composição, interoperação e monitoramento da execução de maneira  automática. OWL-S modela os serviços utilizando uma ontologia, que consiste em três
...
Uma questão fundamental na definição de um serviço composto é saber se os seus serviços componentes podem formar parte uma composição \cite{Dustdar2005a}. Por exemplo,
 pode ser difícil invocar uma operação, se não há uma correspondência
entre os parâmetros necessários para a operação, por exemplo tipos de dados, o número de
...
Atualmente, a  maioria das conexões à Internet são realizadas por dispositivos utilizados diretamente por pessoas, tais como
computadores e telefones celulares. A principal forma de comunicação  é de humano a humano. Em um futuro não muito distante, os objetos
...
O mundo físico e o mundo da informação parecerão um só~\cite{Tan2010}. Nessa ``Internet das Coisas'' (IoT) serão realizadas novas formas de comunicação entre pessoas e coisas, e entre coisas entre si. %Assim, 
...
%    \begin{figure}[!h]
%       \centering
%       \includegraphics[width=0.8\textwidth]{figures/IoT-newDimension.png}
%       \caption{Uma nova dimensão nas tecnologias de Informação~\cite{PapazoglouSOC2007a} }
%       \label{fig:IoT_newDimension}
%     \end{figure}

Não há nenhum padrão para definir a IoT. Considerando a funcionalidade e a identidade como  central,
a IoT pode ser definida como ``Coisas que possuem identidades e personalidades virtuais movimentando-se em espaços inteligentes utilizando
interfaces inteligentes para se conectarem e se comunicarem dentro de contextos de tipo social, de ambiente e de usuário". Uma 
definição diferente com  foco na integração poderia ser formulada como ``Objetos interconectados com um papel ativo no que pode ser
 chamado de `Internet do Futuro' ''~\cite{Tan2010}.
Do ponto de vista conceitual, a IoT baseia-se em três pilares que estão relacionados com a  capacidade dos objetos
 inteligentes de: (i) serem identificáveis, (ii) se comunicarem e
 (iii) interagirem, construindo redes de objetos interconectados, ou com usuários finais ou com outras entidades na
rede~\cite{Miorandi2012}. 

De uma perspectiva de nível de serviço, a questão principal tem a ver com como integrar ou compor as funcionalidades e/ou 
recursos fornecidos por objetos inteligentes dentro de serviços.
Isto requer a definição de: (i) arquiteturas e métodos para virtualizar objetos criando um padrão para representar
  objetos inteligentes no domínio digital, capaz de ocultar a heterogeneidade dos dispositivos e (ii)
métodos para a integração e composição contínua de serviços de objetos inteligentes em serviços de valor agregado
para usuários finais~\cite{Miorandi2012}. As coreografias de serviços Web surgem como tecnologia facilitadora para permitir a integração das diversas coisas na IoT. Além disso, mecanismos como o proposto nesta dissertação são necessários para que as composições de serviço entre as diversas coisas atendam aos requisitos de QoS.
...
O termo IoT é amplamente utilizado para se referir a: (1) o resultado de interconectar
objetos inteligentes em uma rede global por meio de tecnologias da Internet, (2) o conjunto de tecnologias de
...
(3) os conjuntos de aplicações e serviços, aproveitando essas tecnologias para criar novos negócios e 
oportunidades de mercado~\cite{Miorandi2012}.

A tendência de desenvolvimento da IoT inclui três etapas de evolução dos
dispositivos: inteligência embarcada, conectividade e interação. Atualmente,
existe inteligência embarcada em diversos dispositivos e domínios como sistemas
de voo e eletrodomésticos para casa. Porém, esses dispositivos inteligentes apenas trabalham sozinhos e localmente, sem haver comunicação
via rede.

Dessa maneira, o próximo passo seria conectar esses dispositivos inteligentes. No entanto, do ponto de vista de ``dispositivos
inteligentes conectados'', esses dispositivos não são inteligentes porque são apenas dotados com recursos de agentes e todas as ações
 são preconcebidas por humanos.
As coisas podem ser conectadas com fio ou sem fio. Na Internet das Coisas, a conexão sem fio será a principal. Com base 
na infraestrutura existente, há muitas maneiras de conectar uma coisa: RFID, ZigBee, WPAN, WSN, 3G, etc. Conectar coisas inteligentes torna possível a interação.

Mesmo que seja possível conectar qualquer coisa, não significa que as coisas possam se comunicar por decisões próprias. Assim, novas coisas
 inteligentes devem ser criadas com a capacidade de processar informação, se autoconfigurar, se auto-manter, se auto-reparar,
 tomar decisões de forma independente e, eventualmente, desempenhar um papel ativo na sua própria remoção.

%Novos aplicativos de negócios deveriam ser criados com o objetivo de acelerar o desenvolvimento da Internet das Coisas. A
% Figura~\ref{fig:IoT_trends}  mostra uma tendência de desenvolvimento aproximada da Internet de Coisas.
% 
%    \begin{figure}[!h]
%       \centering
%       \includegraphics[width=1.0\textwidth]{figures/IoT-trends.png}
