...
B2E.  %A organização responsável pela definição dessas normativas ou padrões %, e garantir esse comportamento,
 %é o WS-I (\textit{Web Services Interoperability Organization}, \cite{wsi2005}). %Esta organização possibilita
...
\begin{comment}
  Algumas definições de serviço Web são: 
  \begin{itemize}
...
%O modelo de serviços Web acrescenta o desenvolvimento de aplicações distribuídas. 
...
Quanto aos componentes de uma  arquitetura orientada a serviços Web, estão envolvidos os seguintes elementos \cite{Tortosa2006}:
\begin{itemize}
...
mostra-se a pilha de padrões, que oferecem os mecanismos de confiabilidade e qualidade de serviço (QoS: \emph{Quality of Service}): 


    \begin{figure}[!h]
      \centering
      \includegraphics[width=.80\textwidth]{figures/webServicesStackQoS.png}
      \caption{A pilha de tecnologias que garantam a QoS \cite{WSQoS2003}}
      \label{fig:stack_QoS}
    \end{figure}
...
\label{sec:objectives}
%%Os serviços Web são só uma tecnologia para implementar SOA \cite{Rosenberg2009}, no entanto existem outras formas de implementar um SOA
%Segundo \cite{Rosenberg2009}, os sistemas de informação são cada vez mais complexos  e  são necessários sistemas mais exigentes que requerem cooperação. 
%e cada vez mais são  menos comuns as soluções centralizadas, e aparecem  novos métodos de interação (comunicação). O objetivo das organizações é
 %de reduzir custos e maximizar  a utilização da tecnologia existente, entanto as empresas tentam se tornar mais competitivas e avançar na suas prioridades estratégicas. 
...
paradigmas de programação, metodologias de desenvolvimento e também os modelos de computação tanto no hardware quanto nas redes, convergindo para a SOC.
% que se conhece como computação orientada a serviços (SOC).
...
O paradigma da SOC utiliza serviços para suportar o desenvolvimento de maneira rápida, interoperável, evolutiva, e de baixo custo  
para aplicações distribuídas. %Os serviços são entidades independentes de plataforma que são baixo acoplamento e
%podem ser descritos, publicados e  descobertos. 
Os serviços desempenham funções que vão desde responder simples requisições a
executar sofisticados processos de negócios. %, requerendo relacionamentos ponto a ponto (P2P) entre várias camadas de consumidores e provedores de serviços. Qualquer pedaço
 %de código e qualquer componente de aplicação implantado em um sistema pode ser reutilizado e transformado dentro de um serviço e disponibilizado na rede.
...
A promessa visionária de SOC consiste em  construir facilmente  componentes de aplicação dentro de uma  rede de serviços
...
A chave para concretizar a visão de SOC é a arquitetura orientada a serviços (SOA). SOA é uma maneira lógica de desenhar sistemas de software para
...
que separa a funcionalidade em três planos:
serviços básicos na base da pirâmide, composição de serviços no meio e gerenciamento de serviços com monitoramento no topo da pirâmide. Esta divisão lógica está
 baseada na necessidade de separar:
...
\end{itemize}
   ...
   \begin{figure}[!h]
      ...
      \caption{roteiro de pesquisas em SOA ~\cite{PapazoglouSOC2007a} }
