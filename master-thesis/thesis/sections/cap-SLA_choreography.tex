...
%\label{sec:QoS}
% QoS de serviços
Um fator  chave para possibilitar  um comportamento adaptativo em sistemas orientados a serviços, e
...
``conjunto de requisitos de serviços a serem cumpridos pela rede enquanto ela realiza o transporte de um fluxo''. Na comunidade de SOC, a
...
monitoramento e composição de serviços ciente de QoS (\textit{QoS-aware}). Uma qualide do serviço permite
 a otimização de serviços compostos, em termos
da QoS de toda a composição e da adaptação dinâmica de serviços quando acontecerem  mudanças \cite{Rosenberg2009}.
...
feita  em  um monitoramento em tempo de execução. Esses atributos não determinísticos são o foco nesta dissertação, dado que são fatores importantes
para a realização da adaptação dinâmica em sistemas orientados a serviços \cite{Rosenberg2009}. Para os atributos de QoS não
 determinísticos, abordagens de monitoramento são necessárias para avaliar continuamente os valores de atributos de QoS. %atuais.
...
A Figura~\ref{fig:QoSTaxonomy1} exibe uma taxonomia de atributos de QoS que será utilizada
 como referência nesta dissertação. O foco do trabalho será em atributos de desempenho como o tempo de resposta, atraso e largura de banda.
Para o cálculo e estimativa de valores dos atributos de desempenho  serão utilizados os instantes de tempo mostrados 
 na Figura~\ref{fig:QoSTimestamps}, que representa a linha do tempo da invocação de um serviço. A lista a seguir descreve os principais atributos de desempenho de QoS relacionados com a Figura~\ref{fig:QoSTimestamps}:%. A seguir as fórmulas para calcular alguns dos atributos baseados nesses instantes e tempo \cite{Rosenberg2006}:
...
\begin{itemize}
 \item \textbf{Tempo de Processamento}:  Tempo necessário para executar uma operação de um serviço. Dado um serviço $s$ e uma operação $o$, o tempo de
...
\newpage
 \item \textbf{ \emph{Wrapping Time}}: É o tempo para encapsular ou desencapsular uma mensagem.
    ...
    \end{itemize}
  \item \textbf{Tempo de execução}: Tempo que o provedor de serviço gasta para processar uma requisição.
	$$q_{ex}(s,o) = q_{pt} + q_{wp}$$
  \item \textbf{Atraso  ou atraso de rede médio}: Representa o tempo que uma requisição de um serviço demora desde o cliente até
...
\begin{itemize}
% \item \textbf{Disponibilidade}:  Define a probabilidade de um serviço $s$ estar disponível para produzir resultados corretos dentro de um intervalo $[t_0,t_1]$.
%      $$q_{av}(s) = 1 - \frac{t_d}{t_1-t_0}$$
 \item \textbf{Precisão}:  É a taxa de sucesso do serviço $s$. ${r_f}$ é a quantidade de requisições que obtiveram resultados incorretos e ${r_t}$ é a quantidade de requisições totais ao serviço.
      $$q_{ac}(s) = 1- \frac{r_f}{r_t}$$
	
  \item \textbf{Robustez}: Probabilidade de um sistema reagir adequadamente a mensagens de entrada inválidas, incompletas ou conflitantes.
	$$q_{ro}(s) = \frac{1}{r_t}\sum_{i=1}^{n}{f(resp_i(s))}$$
	Onde $resp_i(s)$ é uma função auxiliar, que é a $i-ésima$ resposta produzida por um serviço $s$. $n$ é o numero total de requisições para $s$. $f(m)$
	representa se a resposta de uma mensagem $m$ é válida (retorna 1) ou inválida (retorna 0).
...
%  \section{Outros atributos e métricas de QoS}

  Além da taxonomia apresentada na Figura~\ref{fig:QoSTaxonomy1}, os atributos de qualidade e métricas também podem ser classificados de diferentes perspectivas.
  ...
  As métricas de qualidade podem ser classificadas de acordo com o método em que os dados podem
  ser obtidos. Nesse sentido, três categorias foram identificadas~\cite{Hilari2009}:

  \begin{itemize}
      \item \textbf{Métricas anunciadas pelo Provedor}: Os valores dessas métricas são obtidos pelo anúncio do provedor. Um 
      exemplo seria o custo do serviço.
      \item \textbf{Métricas classificadas pelo Consumidor}: Os valores valores dessas métricas são definidos a partir das opiniões de usuários. Por 
       exemplo, a reputação do serviço obtido a partir de uma média de opiniões dos clientes.
      \item \textbf{Métricas Observáveis}: Os valores dessas métricas são obtidos mediante monitoramento ou testes. Um exemplo 
      é o tempo de resposta ou a disponibilidade.
  \end{itemize}

  Outro tipo de classificação é a partir do fato de que algumas métricas de qualidade podem ser obtidas via
  alguma função de agregação que receba outras métricas de qualidade. Por exemplo, o tempo de resposta médio é a
  média de um conjunto de tempos de resposta atuais em diferentes intervalos de tempo. Essas métricas de qualidade são
  nomeadas métricas derivadas ou calculadas, enquanto as outras são nomeadas métricas básicas ou atômicas.
...
\section{QoS em Sistemas Orientados a Serviços}
 ...
 %(capacidades dos aparelhos   de computação, conectividade de rede)
    o nível de qualidade oferecido pelos serviços de aplicação e por
    características do usuário final como mobilidade, por exemplo. Isso implica que, a fim de se obter uma avaliação exata da
...
integrar coerentemente informação de QoS em vários níveis de abstração: a camada de coreografia, a camada de orquestração e a camada de
 serviços. A Figura \ref{fig:QoS_SLA_MultiLayer} ilustra o modelo com todas as camadas. A seguir é apresentada uma descrição resumida de cada camada. % multicamada para integrar QoS nos níveis de abstração que será implementado neste
...
Os serviços em sistemas orientados a serviços, podem ser invocados por usuários que na maioria dos casos são desconhecidos
 em tempo de projeto e implantação. Por isso, é importante que um serviço forneça uma descrição dos aspectos não funcionais, além dos 
funcionais, por exemplo, em um documento WSDL. Essa camada representa todos os serviços atômicos dentro de um sistema orientado a serviços. Nesta camada
...
A Figura~\ref{fig:QoS_Enabled_WSDL} descreve os componentes de um serviço Web usando um diagrama de classes UML. Esses componentes incluem o
...
serviço associados com o serviço, operação do serviço e as mensagens são resumidos na Tabela~\ref{table:qos_model_wsdl}.


  \begin{figure}[!h]
      \centering
      \includegraphics[width=1.0\textwidth]{./figures/QoS-Model-WSDL.png}
      \caption{ QoS do modelo de serviços Web de acordo com WSDL~\cite{Pandey2010} }
...
agregação de atributos de QoS é realizada por meio de um cálculo incremental baseado em padrões de fluxo de trabalho (\textit{workflows})  bem
...
A definição de SLA e QoS modifica os processos de negócios inter-organizacionais. Cada participante oferece serviços a outros
 participantes sobre a Internet, dos quais os últimos  precisam  executar seus negócios~\cite{Rosenberg2007}. Por isso, um certo grau  de confiabilidade 
com relação ao tempo de resposta, vazão, \textit{uptime}, entre outros, é desejado  e tem de ser especificado e explicitamente
expresso nas etapas iniciais da fase de modelagem. A Figura~\ref{fig:choreographySLA} ilustra uma coreografia com três participantes e seus respectivos
relacionamentos. Para cada interação entre serviços um SLA é definido entre os participantes para definir o que os
participantes precisam realizar para cumprir seus objetivos de negócio.

Dado que uma coreografia de serviços é um descrição de um ponto de vista global das interações dos seus participantes, ela não pode ser executada
diretamente porque não envolve o aspecto interno de um participante. Assim, o SLA precisa ser cumprido também no aspecto interno, chamado de comportamento não-visível, de um participante. Em consequência, uma descrição SLA precisa ser mapeada em politicas concretas
 de QoS na camada de Orquestração.

  \begin{figure}
      \centering
      \includegraphics[width=1.0\textwidth]{figures/ChoreographySLAs.png}
      \caption{Definição de SLAs na camada de coreografias de serviços}
...
Tipicamente, as restrições de QoS são estabelecidas de maneira rígida, isto é, com limites absolutos \cite{Rosario2008}. Por exemplo,
...
entretanto, abordagens específicas para coreografias de serviços Web.%%TODO: quais os desafio inerentes?
...
Existem várias abordagens para integrar modelos de QoS em composição
de serviços, mas poucos focados em coreografias de serviços. WS-CDL
não tem suporte a QoS integrado \cite{Austin2004}. Em \cite{Xiangpeng2007a}, propõe-se um modelo formal que incorpora 
informação de QoS, estendendo a  linguagem
\textit{Chor}\cite{Yang2006}, que é um  WS-CDL simplificado, para adicionar informação de QoS, 
sobretudo tempo e custo, em um nível sintático e semântico para obter a QoS global da coreografia.

Em \cite{Pandey2008}, propõe-se um modelo formal para estimar o mínimo, máximo e a média do custo de uma coreografia. Tal modelo proposto
adiciona características semânticas para incorporar o atributo custo no seu modelo, que é uma extensão do WS-CDL. Este modelo associa o custo com instâncias de um papel. 
Em \cite{Xia2009}, propõe-se uma abordagem para predizer a QoS de uma coreografia de serviços especificada em WSCI. Uma Rede de Petri Estocástica Generalizada
(\textit{GSPN} \footnote{GSPN: Generalized Stochastic Petri Net}) é utilizada como modelo intermediário. A partir do modelo GSPN obtido do mapeamento do modelo WSCI, avaliações
 analíticas da árvore de métricas QoS são realizadas.
Em  \cite{Pandey2010}, é proposto um modelo multidimensional de QoS para coreografias. Tal QoS de coreografia é definido
 em termos do papel  dominante, relacionamento
dominante e  interação dominante. Essas características são avaliadas baseadas na QoS dos papéis, operações e mensagens. Sua proposta é desenvolvida como uma
extensão do metamodelo de WS-CDL para suportar QoS.

Somente em \cite{Rosenberg2009}, propõe-se um  modelo multicamada   para integrar QoS e SLA no nível de coreografia, orquestração e serviços. No nível de coreografias
...
realiza o mapeamento a políticas QoS na camada de orquestração, que por sua vez utiliza os atributos QoS no nível de serviços. Até o presente momento, nenhum
trabalho propõe um modelo de QoS para integrá-lo em BPMN, que é o padrão  para modelar processos de negócios colaborativos, incluindo coreografias.
