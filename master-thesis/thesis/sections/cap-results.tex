...
%%-----------------------------------------------%%
\section{Primeiro conjunto de experimentos -- Definição de Requisitos de QoS Analiticamente }

  Nesta seção todos os passos da metodologia proposta para definir requisitos de QoS de maneira analítica são realizados. A coreografia
  apresentada na Figura~\ref{fig:Example-InteractionChor} é utilizada. Este cenário foi escolhido porque  inclui os três tipos elementos de BPMN de coreografias
  considerados (atividades, \textit{gateways} e eventos). As próximas subseções desenvolvem cada um dos passos da metodologia.
...
\subsection{ Mapeamento } \label{subsec:mapping_performance}
  ...
  sobre a  coreografia da Figura~\ref{fig:Example-InteractionChor}. As Tabelas~\ref{table:place_description} e \ref{table:transition_description}
   mostram a interpretação de todas as posições e transições da GSPN resultante. As posições representam o início e fim da coreografia, os participantes, 
 o início e fim do canal de comunicação nas interações,  pontos de decisão e os diversos erros de acordo com o modelo de QoS.
  ...
  \begin{table}[!h]
	...
	Posição     &       Descrição \\
	\hline
	$Start$, $End$            &  Início e fim da coreografia. \\
  ...
  \begin{table}[!h]
	...
	$T_{arrival2}$, $_3$   & Chegada de instâncias de coreografia e decisões de fluxo.\\
  ...
  O próximo passo é definir pesos das transições de tempo e as prioridades nas transições imediatas. Esses valores
 dependem do conhecimento acerca do ambiente onde as coreografias serão executadas e da confiabilidade dos serviços. 
 ...
 A Tabela~\ref{table:transitionsConfigurations} apresenta a definição dos pesos e prioridades em dois cenários, os quais serão simulados no próximo passo. 
  ...
  Na simulação, 100 \textit{tokens} foram considerados para cada cenário na Posição de Início. Cada \textit{token}
  representa uma instância de coreografia na simulação. Assim, para cada simulação, 100 instâncias concorrentes dos
  cenários foram  executados.
  %Since  there are probabilities  involved in the experiments
  As simulações dos dois cenários foram executadas várias vezes de acordo com as configurações estabelecidas na
  ...
  na tabela mostram a média do número de \textit{tokens} que chegaram em cada uma das Posições da GSPN.
  ...
  das instâncias não finalizaram o processo devido a erros de rede no momento da troca de mensagens. Além disso, uma média de  $0.39\%$ ($M_{err1} + M_{err2} + M_{err3}$)
  das instâncias não finalizaram o processo devido a formatos de mensagens inválidos, que foram detectados quando as mensagens foram recebidas.
  ...
  No cenário 2, uma média de $3.10\%$ e $2.50\%$ das instâncias não finalizaram o processo devido a erros na rede 
  e a formatos de mensagens inválidos, respectivamente. Como esperado, os resultados estão de acordo com os parâmetros definidos
  para cada cenário (Tabela~\ref{table:transitionsConfigurations}).

  %One important finding
  Um importante resultado com as simulações desses cenários, e que  pode ser útil para desenvolvedores e administradores de rede 
  está relacionado com a existência de gargalos na primeira interação ($C_{i1}$), a qual evitou que $8.32\%$ e $8.90\%$ das
  instâncias fossem enviadas pela rede. Essa  informação pode afetar a modelagem e a implementação da coreografia definitiva, bem como
  definir novas políticas de QoS para serem configuradas na rede com o objetivo de reduzir o atraso na execução.
...
%  A Figura~\ref{fig:preliminarResults1} mostra o tempo de resposta
...
%     \caption{  Tempos de reposta médio, máximo e minimo de acordo ao número de requisições concorrentes. }
...
%%-----------------------------------------------%%
\section{Segundo conjunto de experimentos -- Definição de Requisitos de QoS em Coreografias}%%Simulator
  ...
  Neste conjunto de experimentos, a coreografia apresentada em \cite{Buccafurri2008a} é utilizada. Essa coreografia representa uma aplicação de
  CDN (\textit{Content Delivery Network}) para fornecimento de conteúdo multimídia como áudio, vídeo e imagens. 
A Figura~\ref{figure:scenario} mostra o cenário de referência: um usuário requere, e eventualmente recebe, 
um serviço complexo gerido por meio de uma coreografia de diferentes serviços Web, um dos quais ($WS_3$ na figura) controla o provisionamento
 de conteúdo de  \textit{streaming}. $WS_1$ e $WS_2$ mostram que vários serviços Web são internamente orquestrados. 
%A Figura ~\ref{figure:scenario} ilustra  a coreografia.  % a simular mostrando os serviços e suas respectivas interações.
A coreografia é composta de cinco serviços  Web ($WS_1$,$WS_2$, $WS_3$, $WS_4$ e $WS_5$ ). Cada serviço pertence a um participante
  ...
  Porém, nem sempre todos os serviços  de uma coreografia são utilizados em uma instância de coreografia, isto é, há interações entre serviços
  ...
  \end{figure}
    ...
    O objetivo dos experimentos apresentados nesta seção é analisar o comportamento do tempo de resposta total do serviço composto $WS_1$ em função
    ...
    e de acordo à largura de banda da rede. %,  abrangendo uma alta capacidade (H),  de média capacidade (M) e de
    ...
    valores dos atributos de QoS das requisições e das respostas que são usados para as simulações. Diferente do primeiro conjunto de experimentos, estes experimento foram simulador no ChorSim, e não utilizando o Pipe2.
    ...
    processador Intel Core $i7-2700$K $3.5$Ghz, $16$GB de memória RAM rodando o sistema operacional
	...
	\begin{figure}[!h]
	    ...
	    \caption{Modelo de falhas que mostra a largura de banda efetiva devido à degradação da largura de banda em um período de $100$ segundos.}
    ...
    resposta é menor quanto  maior é a largura de banda, e estabiliza-se a partir de $5Mbps$ com um tempo de resposta
      de cerca de $150ms$. Além disso, com valores pequenos  nos tamanhos de resposta de até $30MB$ os tempos de resposta são
    similares com larguras de banda a partir de $2Mbps$. Com base nisso já pode-se definir restrições de QoS tais como, poder
    oferecer tempos de resposta menores que $200ms$  desde que se tenha uma infraestrutura para suportar uma largura de banda de mais de $5Mbps$. %%

    Já no modelo variável com falhas, os tempos de resposta continuam diminuindo quanto maior é a largura de banda, mas diferente do modelo normal,
    neste cenário o tempo começa a se estabilizar a partir de $14Mbps$ de largura de banda, apesar de a partir dos $8Mbps$ os tempos de resposta
...
%%-------------------------------------------------%%
\section{Terceiro conjunto de experimentos -- Estabelecimento de contratos de QoS}%%Simulator

Nesta etapa, um cenário similar ao do segundo conjunto de experimentos é utilizado. A diferença está na largura de banda nas respostas de $WS_1$ para o cliente, que foi definida como $10Mbps$. Vamos considerar que a degradação do tempo de processamento ou execução dos serviços individuais $WS_1$ e $WS_3$ e $WS_5$ se comporta de acordo com
 distribuições exponenciais. A escolha foi feita porque não queremos considerar que há memória em relação à falhas que aconteceram no passado, o que é possível de ser simulado com uma distribuição exponencial.  Para todos os serviços a taxa de degradação, que será a média da distribuição exponencial, foi considerada como 1/10000.

%A Tabela~\ref{table:contract_configurations}
%apresenta o tipo de distribuição de probabilidade usado e a taxa de degradação para os tempos de execução dos serviços $WS_1$ e $WS_3$ e $WS_5$.
% O resto da configuração é  igual como é descrevido nas tabelas \ref{table:simulation_configuration_requests} e
% \ref{table:simulation_configuration_responses}, exceto que a largura de banda nas repostas do $WS_1$ para cliente foi definida para ser  de $10 Mbps$ 
...
Após obter por meio do ChorSim um conjunto de  $5000$ amostras dos tempos de resposta  do serviço composto $WS_1$, já é possível
obter a distribuição de probabilidade empírica (ECDF) $F_{ws1}$ que represente o contrato. A Figura~\ref{figure:contract} mostra a ECDF obtida
...
%%---------------------------------------------------%%
\section{Quarto conjunto de experimentos -- Monitoramento de QoS em Coreografias}%%Simulator
  ...
  tais como o tamanho  da janela de amostras $N$, o desvio da janela de amostras $p$,  e os parâmetros relacionados à regra
...
A regra específica para o cumprimento de garantia de QoS precisa  definir os parâmetros  de zona de tolerância $\lambda$ e o nível de significância
 $\alpha$. Desse modo, a regra de garantia fica da seguinte forma:
...
Onde $X_{monitoring}$ é o conjunto de amostras dos tempos de resposta do serviço composto $WS_1$. Para habilitar um monitoramento sequencial e
 \textit{on-line} precisa-se computar um conjunto de amostras sequenciais de tamanho $N$ que se sobrepõem. 
Assim, o conjunto de janelas de  amostras a monitorar seria:  $\{1, \dots , N\}$, $\{p, \dots , p + N\}$, $\dots$ , $\{mp, \dots ,mp + N\}$,
e assim por diante, onde $p <=N $ e $m=1, 2,\dots \quad$ . O tamanho de amostras ($N$) para realizar a detecção  é de 100, e o desvio $p$ é 1.
 

  \begin{center}
    \begin{table}[h]
	\centering
      \caption{Definição de valores na configuração para o monitoramento. }
      \begin{center}
	\begin{tabular}{|c|c|}
	  \hline
	  % after \\: \hline or \cline{col1-col2} \cline{col3-col4} ...
...
apresentar diversos comportamentos nos tempos de resposta e na detecção de violações do contrato estabelecido. Tais cenários são caracterizados
...
mostras os valores das taxas de degradação dos serviços Web para quatro cenários, onde do primeiro até o terceiro cenário  há taxas de degradação
 maiores  do que as degradações que foram usadas para o estabelecimento do contrato, mas que vão se aproximando como no terceiro cenário. O quarto
 cenário possui taxas de degradação de desempenho que são um pouco menores do que as usadas  no estabelecimento do contrato.
...
Para cada um dos quatro cenários, foram executadas simulações de $1000$ requisições para o serviço composto $WS_1$. Além disso, a topologia e configuração
da implantação (largura de banda, latência, capacidade de processamento, etc) é a mesma como definida no estabelecimento de contrato probabilístico. %Os
% apêndices~\ref{ape:chor_specification},~\ref{ape:platform} e ~\ref{ape:deployment} 
% mostram com mais detalhes a especificação da coreografia, da plataforma e da implantação que o ChorSim precisa para rodar as simulações. 
  Os gráficos~\ref{figure:detection1-13_125_115}, \ref{figure:detection2-12_11_12},~\ref{figure:detection4-105_10_105} 
...
contrato e ressalta-se os valores de $\lambda$ e $\alpha$ com uma
linha horizontal vermelha para cada um. Esses gráficos mostram a
distância do teste de Kolmogorov, o qual mostra o quanto flutua a
distribuição  dos tempos de resposta em relação ao contrato e de acordo com 
a regra  \eqref{eqn:rule_detection} deve ser menor que $0.15$  para
ser cumprida. Os gráficos também mostram os \textit{$p^{+}$-values} para cada uma das
 requisições realizadas, e de acordo com a regra em \eqref{eqn:rule_detection} devem ser maiores ou iguais que $0.05$ para cumprir o contrato.
...
%%Análise
No cenário $1$ houve violações do contrato em $92.33\%$ dos casos, o que se evidencia pelos vários \textit{p-values} menores do que $0.05$ e flutuações
maiores do que a zona de tolerância $\lambda$ definida. O contrato foi
garantido apenas no intervalo entre as requisições  $839$ e $926$,
 onde as flutuações tem maior queda  e os \textit{p-values} atingem valores altos.  
No cenário $2$ houve violações em $85.33\%$ dos casos, onde o contrato foi garantido poucas vezes e principalmente nos intervalos
 das requisições de $103$ a $163$, de $601$ a $618$ e de $947$ a $1000$. Como no primeiro cenário, as requisições que cumpriram
 com o contrato  coincidem em lugares onde há flutuações com picos baixos  e \textit{p-values} com  picos altos.
%%estes dois cenários se detectaram muitas violações de contrato por conta das taxas de degradação altas dos serviços 

Diferente dos cenários anteriores, o cenário $3$ apresenta violações
de contrato em menos casos ($28.22\%$ dos casos). 
No cenário $4$ as violações de contrato ocorreram em $1\%$ das $1000$ requisições. Dessa vez não houve \textit{p-values} menores que $0.05$, mas 
  as flutuações foram determinantes para detectar as poucas violações. No entanto, dado que neste cenário as taxas de degradação do
 tempo de processamento dos serviços é menor do que no contrato, não
devia-se detectar falhas mesmo sendo $1\%$. Então, para evitar 
esses falsos alarmes poderia se usar um valor $\lambda$ ( zona de tolerância) maior do que definido na regra atual.   Por exemplo,
poderíamos estabelecer $\lambda= 0.158$ porque é a maior distância
achada nos casos que quebraram o contrato.
...
%  probabilidade, que já é suportado no \textit{ChorSim}. Desse modo, no estabelecimento e no monitoramento usou-se taxas de degradação do desempenho por meio de
...
% está de outra e já que é de tipos não paramétrico podem ser testados diversos tipo de distribuições.  
