...
\section{Cenários de Coreografia}
  ...
  CDN (\textit{Content Delivery Network}) para fornecimento de conteúdo multimídia como audio, vídeo e imagens. 
...
Nesta etapa, vamos supor que a degradação do tempo de procesamento ou execução dos serviços individuais $WS_1$ e $WS_3$ e $WS_5$ se comportam de acordo com
 distribuições exponenciais. A escolha foi feita porque uma distribuição exponencial descreve o tempo para que um processo mude de estado,
...
mostras os valores das taxas de degradação dos serviços Web para quatro cenários, onde do primeiro até o terceiro cenário  possuem taxas de degradação
 maiores  do que as degradações que foram usadas para o estebelecimento do contrato, mas que vão se aproximando como no terceiro cenário. O quarto
 cenário possui taxas de degradação de desempenho que são um pouco menores do que as usadas  no estabelecimento contrato.
...
Para cada um dos quatro cenários, o monitoramento se executou com  simulações de $1000$ requisições para o serviço composto $WS_1$. Além disso, a topologia e configuração
da implantação (largura de banda, latência, capacidade de procesamento, etc) é a mesma como definida no estabelecimento de contrato probabilístico. Os
...
%%Análise
No cenário $1$ houve violações do contrato no $92.33\%$ dos casos, o que se evidencia pelos vários \textit{p-values} menores do que $0.05$ e flutuações
...
No cenário $2$ houve violações no $85.33\%$ dos casos, onde a o contrato foi garantido poucas vezes e principalmente nos intervalos
...
se correspondem nos intervalos onde há  picos altos nas flutuações. Apesar que dessa vez os \textit{p-values} que quebram o contrato estão 
...
No cenário $4$ se detectaram violações de contrato no $1\%$ das $1000$ requisições. Dessa vez não houve \textit{p-values} menores que $0.05$, mas 
  as flutuações foram determinates para detectar as poucas violações. No entanto, dado que neste cenário as taxas de degradação do
...
poderiamos estabelecer $\lambda= 0.158$ porque é a maior distância achada das que quebraram o contrato.
