...
%, connection timed out, and so on .  
 ...
 conducts a performance evaluation by means of generalized stochastic Petri nets (GSPN) as the intermediate representation of a choreography
...
\section{Basic Concepts } \label{sec:basicConcepts}
 ...
 from orchestrations, choreographies don't have a single point of control or coordination \cite{Barker2009}. Choreography tend to be more
...
Recently, \cite{Xia2011} proposed an approach to predict the reliability of composite services specified in WS-CDL. It is also
 ...
 of Petri nets as an intermediate representation of a choreography. \cite{Xia2011} is focused in reliability, while our proposal is focused in
...
The contribution of this paper is to present a new methodology for evaluate the performance of choreographies with the objective of
...
\begin{enumerate}
 ...
 \item Simulations of scenarios, one with a minimum configuration of failures and another with a higher possibility of failure.
...
%. Para tanto, a seguir vamos definir o \textit{core process choreography BPMN} (baseado em \cite{Dijkman2007}).
    ...
    \begin{mydef}
      A choreography of BPMN processes is a tuple: \\
       $PC = (\mathcal{O, A, E, G, T, S}, \{e^S\}, \mathcal{E}^I, \{e^E\}, \mathcal{E}^{I_M}, \mathcal{E}^{I_T}, \mathcal{G}^F, \mathcal{G}^J,$
	...
	\item $\mathcal{A}$, is the set of choreography tasks and it is the same as $\mathcal{T}$.
...
%Basicamente, a GSPN da coreografia toda é obtida mediante a composição dos módulos de GSPN.
	...
	\REQUIRE Process Choreography $PC = (\mathcal{O, A, E, G, T, S}, \{e^S\}, \mathcal{E}^I,$ $\{e^E\}, \mathcal{E}^{I_M}, \mathcal{E}^{I_T},
	...
	\FOR { $CT_i \in \mathcal{T} $ }
	  \STATE $GSPN_{QoS} \leftarrow GSPN_{QoS} \oplus PNQoS(CT_i)$
          \STATE Add a arrival timed Transition at first of the $GSPN_{QoS}$.
	\ENDFOR
	...
	\STATE Add a starting Place and immediate Transition at the beginning of the $GSPN_{QoS}$.
        \STATE Add a ending Place and immediate Transition the end of the $GSPN_{QoS}$.
...
To attest the efficacy of our proposal, in this section all the steps of our methodology are performed. The choreography
 presented in Figure~\ref{fig:choreographyExample} is used in the process. Each one of the next subsections describes one of the three steps of 
 the proposal defined in the Section~\ref{sec:proposta}.
...
\begin{table}[!h]
    ...
    \begin{tabular}{|l|l|}
      \hline
       ...
       $C_{error1}$    & Communication error  between A and B. \\
       $C_{error2}$    & Communication error  between B and A. \\
       $C_{error3}$    & Communication error  between B and A. \\
...
\begin{table}[!h]
    ...
    \begin{tabular}{|l|l|}
      \hline
       ...
       $T_{arrival1}$, $_2$, $_3$  &  Choreography instances arrival.\\
       $T_{arrival2}$, $_3$   &  Flow decision.\\
       $T_{latency1}$, $_2$, $_3$   &     Network latency\\
       $T_{receive1}$, $_2$, $_3$   &     Receive and service execution.\\	
       $T_{cerror1}$, $_2$, $_3$  &  Communication error. \\
       $T_{merror1}$, $_2$, $_3$ &  Format fault message. \\
...
The next step is to define the weights representing the timed transitions and the priorities in the immediate transitions. These values
 ...
 the definition of the weights in two scenarios, that will be needed to the simulations on the next step. Scenario 1 is configured
...
\begin{table}[!h]
    ...
    \begin{tabular}{|l|l|l|}
      ...
      %\hline
       ...
       $T_{cerror1}$, $T_{cerror2}$, $T_{cerror3}$      	&  	0.01 	   &   0.06	\\
       $T_{receive1}$, $T_{receive2}$, $T_{receive3}$  &  	99  	   &    97	\\
...
To simulate the scenarios, 100 tokens are considered to each scenario at the place Start. Each token represent one instance of execution
 of the simulation. So, in each simulation, 100 concurrent instances of the scenarios were executed. Since there are probabilities
 involved in the experiments, as seen in Table~\ref{table:transitionsConfigurations}, the experiments need to be executed several times. There were defined 1500
 fires and 10 replications. To simulate the scenarios, the simulator Pipe2 \cite{Bonet2007} was used.

 Table~\ref{table:simulationsAll} shows the results of the simulations. The numbers presented in the table show the average of the tokens 
that arrived at each one of the places of the GSPN.
...
\begin{table}[!h]
    ...
    \begin{tabular}{|l|l|l|l|l|}
	  ...
	  \hline
	  $Start$ &    35,277	 &    40,14833        &   5,83475    	 &    6,23113         \\
	  $End$   &    41,94523	 &    38,78167        &   2,53329  	  &  3,82088	\\
	  $A_1$   &    0,07825	 &    0,08333         &   0,00105   	&  0,00369     \\ %% após da execucao do Treceive 1
	  $B_2$	  &    0,03599	&     0,00469 		& 	0,00604 &  0,00651 \\ %% após da execucao do Treceive 2
	  $B_3$	  &    0,03991	&     0,0099  		& 0,00643 	&  0,00742 \\ %% após da execucao do Treceive 3
      \hline
	  $B_1$	  &    0,07668	 &  3,22932  		& 0,00056	&  0,00168	\\ %Primeiro participante em iniciar a interação
	  $A_2$	  &    0,03599	 &  0,00843		& 0,00643	&  0,00724	\\ %% aceito
	  $A_3$	  &    0,04069	 &  0,00531		& 0,00612	&  0,00638	\\ %%rejeitado
        \hline
	  $M_{err1}$	& 0,39202	&     0,90583 		& 0,94914	&  1,92248	\\
	  $M_{err2}$	& 0	 &    0,9325 		& 0,62632		&  0,63929	\\
	  $M_{err3}$	& 0   & 0,65583 		& 0,86536	&  0,74018	\\
        \hline
	  $C_{err1}$	& 0,73787 	 & 2,93833 	& 0,81703	&  2,02391	\\
	  $C_{err2}$	& 0	         & 0 		& 0,66667	&  1,75059	\\
	  $C_{err3}$ 	& 0,77856	 & 0.16233 	& 0,91667	&  1,51997	\\
        \hline
	  $C_{i1}$ 	& 8,32316	 & 8,90167 	& 5,32508	&  7,47872	\\
      $C_{i2}$ 	& 0,62676	 & 0,6875 	& 0,22787	&  0,52219	\\
      $C_{i3}$ 	& 0,75039	 & 8,90167 	& 0,39	&  0,21236	\\
...
By the results presented in Table~\ref{table:simulationsAll} it is possible to note that in the Scenario 1, an average of $1.51643\%$ ($Cerr1 + Cerr2 + Cerr3$) of 
the instances didn't  finish the process because of error in the network in the moment of the message exchanges. Also, an average
 of $0,39202\%$ ($Merr1 + Merr2 + Merr3$) of the instances didn't finish the process because of error in the messages
...
Similarly to the Scenario 1, in Scenario 2 $4,52167\%$ and $2,49416\%$ of the instances didn't finish the process because of error in 
...
bottleneck in the first iteration ($C_{i1}$), which avoided $8,32316\%$ and $8,90167\%$ of the instances for being send via
